...
import { ArrowLeft, User, BarChart3, ClipboardList, Target, CheckCircle2, Sparkles, Star, Calendar, Award, Code2, Globe, RefreshCw, Download } from 'lucide-react';
...
export default function StudentDetail() {
  ...
  const exportFullReport = async () => {
    ...
    const toastId = toast.loading('Compiling beautifully formatted PDF report...', {
      style: {
        background: '#1e293b',
        color: '#f8fafc',
  ...
  return (
    <div className="md:p-10 pb-20 relative">
      <div className="sticky top-0 z-[100] -mx-6 md:-mx-10 px-6 md:px-10 py-5 bg-slate-900/80 backdrop-blur-xl border-b border-white/[0.05] mb-8 shadow-lg shadow-black/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
          <Link to="/students" className="shrink-0 p-2.5 rounded-xl bg-white/[0.03] hover:bg-white/[0.08] text-slate-400 hover:text-white transition-all ring-1 ring-white/5 hover:ring-white/20 shadow-sm self-start sm:self-center mt-2 sm:mt-0">
            <ArrowLeft className="w-5 h-5" />
          </Link>
          
          <div className="flex items-center gap-5">
            <div className="relative shrink-0">
              <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-xl sm:text-2xl font-black text-white shadow-xl shadow-indigo-500/20 ring-2 ring-white/10">
                {student.name.substring(0, 2).toUpperCase()}
              </div>
              <div className="absolute -bottom-1 -right-1 w-5 h-5 sm:w-6 sm:h-6 rounded-lg bg-gradient-to-br from-emerald-400 to-green-500 flex items-center justify-center border-2 border-slate-900">
                <Sparkles className="w-3 h-3 text-white" />
              </div>
            </div>
            
            <div className="flex flex-col justify-center">
              <h1 className="text-2xl sm:text-4xl font-extrabold text-white tracking-tight leading-none mb-3 sm:mb-4">{student.name}</h1>
              <div className="flex items-center gap-2 flex-wrap">
                <span className={`text-[9px] sm:text-[10px] uppercase tracking-[0.1em] font-extrabold px-2.5 py-1 rounded-md border ${isMERN ? 'border-blue-500/30 text-blue-400 bg-blue-500/10' : 'border-emerald-500/30 text-emerald-400 bg-emerald-500/10'}`}>
                  {student.track || 'Unassigned'}
                </span>
                <span className="text-[10px] text-slate-500 font-mono tracking-wider font-medium">#{student.id.toString().padStart(3, '0')}</span>
                {insights.strengths.map((t) => (
                  <span key={`s-${t}`} className="px-2.5 py-1 rounded-md text-[9px] sm:text-[10px] font-extrabold tracking-wide border bg-emerald-500/10 text-emerald-400 border-emerald-500/20">
                    {t}
                  </span>
                ))}
                {insights.weaknesses.map((t) => (
                  <span key={`w-${t}`} className="px-2.5 py-1 rounded-md text-[9px] sm:text-[10px] font-extrabold tracking-wide border bg-rose-500/10 text-rose-300 border-rose-500/20">
                    {t}
                  </span>
                ))}
              </div>
        ...
        <div className="bg-slate-800/50 p-1.5 rounded-xl flex items-center gap-1 border border-white/10 shrink-0 w-full sm:w-auto overflow-x-auto">
          <button
            onClick={() => setTermFilter('all')}
            className={`px-4 sm:px-5 py-2 text-xs font-bold rounded-lg transition-all whitespace-nowrap ${termFilter === 'all' ? 'bg-[var(--color-filter-indigo)] text-[var(--color-filter-text)] shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          >
            All Weeks
          </button>
          <button
            onClick={() => setTermFilter('mid')}
            className={`px-4 sm:px-5 py-2 text-xs font-bold rounded-lg transition-all whitespace-nowrap ${termFilter === 'mid' ? 'bg-[var(--color-filter-cyan)] text-[var(--color-filter-text)] shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          >
            Mid Term (W1-W4)
          </button>
          <button
            onClick={() => setTermFilter('last')}
            className={`px-4 sm:px-5 py-2 text-xs font-bold rounded-lg transition-all whitespace-nowrap ${termFilter === 'last' ? 'bg-[var(--color-filter-purple)] text-[var(--color-filter-text)] shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          >
            Last Term (W5-W8)
          </button>
        </div>
        ...
        {/* <button 
          ...
          className={`w-full sm:w-auto px-6 py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all group shrink-0 ${isExporting ? 'bg-indigo-500/20 text-indigo-300 cursor-not-allowed border border-indigo-500/30' : 'bg-white/[0.03] hover:bg-indigo-500/10 text-white border border-white/10 hover:border-indigo-500/50 shadow-sm hover:shadow-indigo-500/20'}`}
        >
          {isExporting ? (
            <><RefreshCw className="w-4 h-4 animate-spin" /> Compiling...</>
          ) : (
            <><Download className="w-4 h-4 text-indigo-400 group-hover:scale-110 transition-transform" /> Export Report</>
          )}
        </button> */}
      </div>
